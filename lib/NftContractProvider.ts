// The name below ("DeixaXastrosCollection") should match the name of your Solidity contract.
// It can be updated using the following command:
// yarn rename-contract NEW_CONTRACT_NAME
// Please DO NOT change it manually!
import { DeixaXastrosCollection as ContractType } from '../typechain/index';
...
export default class NftContractProvider {
  public static async getContract(): Promise<ContractType> {
    ...
    if (null === CollectionConfig.contractAddress) {
      throw '\x1b[31merror\x1b[0m ' + 'Please add the Collection contract address to the configuration before running this command.';
    }

    if (await ethers.provider.getCode(CollectionConfig.contractAddress) === '0x') {
      throw '\x1b[31merror\x1b[0m ' + `Can't find a Collection contract deployed to the target address: ${CollectionConfig.contractAddress}`;
    }
